...
namespace poly {
   ...
   size_t device_context::malloc_scene(std::shared_ptr<scene> scene) {
      ...
      cuda_check_error( cudaMalloc((void **)&(host_samples.r), sizeof(float) * pixel_count) );
      to_free_list.push_back(host_samples.r);
      cuda_check_error( cudaMalloc((void **)&(host_samples.g), sizeof(float) * pixel_count) );
      to_free_list.push_back(host_samples.g);
      cuda_check_error( cudaMalloc((void **)&(host_samples.b), sizeof(float) * pixel_count) );
      ...
      // meshes
      constexpr size_t device_mesh_size = sizeof(struct device_mesh);
      
      struct device_mesh* host_meshes_temp = nullptr;
      ...
      for (auto original_mesh : scene->Shapes) {
         ...
         try {
            ...
            to_free_list.push_back(device_mesh_geometry_temp.x);
            cuda_check_error( cudaMemcpy(device_mesh_geometry_temp.x, &(host_mesh->mesh_geometry->x[0]), num_bytes, cudaMemcpyHostToDevice) );

            bytes_copied += num_bytes;

            cuda_check_error( cudaMalloc((void **)&(device_mesh_geometry_temp.y), num_bytes) );
            to_free_list.push_back(device_mesh_geometry_temp.y);
            cuda_check_error( cudaMemcpy(device_mesh_geometry_temp.y, &(host_mesh->mesh_geometry->y[0]), num_bytes, cudaMemcpyHostToDevice) );

            bytes_copied += num_bytes;

            cuda_check_error( cudaMalloc((void **)&(device_mesh_geometry_temp.z), num_bytes) );
            to_free_list.push_back(device_mesh_geometry_temp.z);
            cuda_check_error( cudaMemcpy(device_mesh_geometry_temp.z, &(host_mesh->mesh_geometry->z[0]), num_bytes, cudaMemcpyHostToDevice) );
            ...
            if (host_mesh->mesh_geometry->has_vertex_normals) {
               ...
               to_free_list.push_back(device_mesh_geometry_temp.nx);
               cuda_check_error( cudaMemcpy(device_mesh_geometry_temp.nx, &(host_mesh->mesh_geometry->nx[0]), num_bytes, cudaMemcpyHostToDevice) );

               bytes_copied += num_bytes;

               cuda_check_error( cudaMalloc((void **)&(device_mesh_geometry_temp.ny), num_bytes) );
               to_free_list.push_back(device_mesh_geometry_temp.ny);
               cuda_check_error( cudaMemcpy(device_mesh_geometry_temp.ny, &(host_mesh->mesh_geometry->ny[0]), num_bytes, cudaMemcpyHostToDevice) );

               bytes_copied += num_bytes;

               cuda_check_error( cudaMalloc((void **)&(device_mesh_geometry_temp.nz), num_bytes) );
               to_free_list.push_back(device_mesh_geometry_temp.nz);
               cuda_check_error( cudaMemcpy(device_mesh_geometry_temp.nz, &(host_mesh->mesh_geometry->nz[0]), num_bytes, cudaMemcpyHostToDevice) );

               bytes_copied += num_bytes;
            }
            
            // 2. copy it to the device
            cuda_check_error( cudaMemcpy(device_mesh_geometries_temp + next_device_mesh_geometry_index, &device_mesh_geometry_temp, sizeof(struct device_mesh_geometry), cudaMemcpyHostToDevice));
         ...
         struct device_mesh host_mesh_temp { };
...
//         cuda_check_error( cudaMemcpy(host_mesh_temp.src, &(host_mesh->src[0]), num_bytes, cudaMemcpyHostToDevice) );
         ...
         cuda_check_error( cudaMemcpy(host_meshes_temp + device_mesh_offset, &host_mesh_temp, device_mesh_size, cudaMemcpyHostToDevice));
